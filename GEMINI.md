...
Обязательная ручная проверка перед коммитами или в конце задач (соответствует CI/CD).
...
```
*(Успех: нет ошибок от ruff и mypy)*

**Frontend (Vue/Nuxt 3):**
```bash
cd frontend
npm install --quiet
npm run lint # запускает vue-tsc --noEmit
```
