...
class FirmwareRepository:
    ...
    async def list_all_devices(self) -> Sequence[ModuleDevice]:
        stmt = (
            select(ModuleDevice)
            .options(selectinload(ModuleDevice.complectations), selectinload(ModuleDevice.token).selectinload(ModuleToken.user))
        )
        result = await self.session.execute(stmt)
        return result.scalars().all()
    ...
    async def get_complectation_by_code(self, code: int, simple: bool = False) -> Optional[ModuleComplectation]:
    ...
    async def add_complectation_to_device(self, serial: str, comp_id: uuid.UUID):
        ...
        stmt = device_complectations.insert().values(device_serial=serial, complectation_id=comp_id)
        await self.session.execute(stmt)

    async def merge_tokens(self, source_user_id: uuid.UUID, target_user_id: uuid.UUID):
        # 1. Get tokens
        ...
        if not target_token:
            # Transfer source token to target user
            source_token.user_id = target_user_id
            await self.session.flush()
        else:
            # Move all devices from source token to target token
            stmt = (
                update(ModuleDevice)
    ...
    # User helper for import (simplified)
