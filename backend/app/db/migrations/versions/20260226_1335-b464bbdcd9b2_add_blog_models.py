...
def upgrade() -> None:
    # Check if blogstatus type exists
    res = op.get_bind().execute(sa.text("SELECT 1 FROM pg_type WHERE typname = 'blogstatus'"))
    if not res.first():
        sa.Enum('draft', 'published', 'archived', name='blogstatus').create(op.get_bind())
    ...
    sa.Column('status', postgresql.ENUM('draft', 'published', 'archived', name='blogstatus', create_type=False), nullable=False),
