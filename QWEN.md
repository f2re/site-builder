...
> Текущий спринт / активные задачи: [.qwen/agents/tasks/](.qwen/agents/tasks/)
...
5. Каждый агент **MUST** писать отчёт в `.qwen/agents/reports/<domain>/<task_id>.md`
...
Полный граф зависимостей фаз: см. [orchestrator.md](.qwen/agents/orchestrator.md)

### Точка входа для новой задачи
```
/agents:plan <описание задачи>
```
Оркестратор декомпозирует задачу, создаёт `.json`-файлы в `.qwen/agents/tasks/` и строит план выполнения.
