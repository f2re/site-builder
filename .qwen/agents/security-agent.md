...
- [ ] No PD in application logs
- [ ] User delete endpoint exists: DELETE /api/v1/users/me
- [ ] Privacy policy endpoint exists: GET /api/v1/users/privacy

### Dependency Vulnerabilities
```bash
safety check -r backend/requirements.txt --output json > .qwen/agents/reports/security/safety_backend.json
bandit -r backend/app/ -ll -f json -o .qwen/agents/reports/security/bandit.json
```
...
```bash
cd frontend
npm audit --json > ../.qwen/agents/reports/security/npm_audit.json
...
- [ ] No API keys/secrets in source: `grep -rn "api_key|secret|password" frontend/`
...
- [ ] v-html usage audited - only allowed with sanitised content
...
```bash
npx axe-cli http://localhost:3000 --exit --reporter json > .qwen/agents/reports/security/axe_dark.json
npx axe-cli "http://localhost:3000?theme=light" --exit --reporter json > .qwen/agents/reports/security/axe_light.json
```
...
Write report to `.qwen/agents/reports/security/<task_id>.md`:
